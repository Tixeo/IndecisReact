...
import { Search, Filter, Clock, BookOpen, Lock, BookMarked } from "lucide-react"
...
export default function GuidesPage() {
  ...
  const { articles, isLoading, error } = useArticlesByCategory("guides")
  ...
  const filteredGuides = articles ? articles.filter(guide => {
    const matchesSearch = guide.title.toLowerCase().includes(searchQuery.toLowerCase()) || 
                          guide.description.toLowerCase().includes(searchQuery.toLowerCase())
    const matchesTag = activeTag === "all" || guide.tag.toLowerCase() === activeTag.toLowerCase()
          ...
          <div className="container">
            <div className="mx-auto max-w-[800px] text-center">
              ...
              <p className="mb-8 text-muted-foreground md:text-xl">
                Detailed guides to help you master all aspects of managing a Discord server.
              </p>
          ...
          <div className="container">
            ...
            {isLoading ? (
              ...
              <div className="text-center py-10">
                <p className="text-lg text-red-500">Error loading guides. Please try again later.</p>
              </div>
            ) : (
              <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {filteredGuides.length === 0 ? (
                  <div className="col-span-full text-center py-10">
                    <p className="text-lg text-muted-foreground">No guides match your search.</p>
                  </div>
                ) : (
                  filteredGuides.map((guide) => (
                    <Link href={`/guides/${guide.numeric_id}`} key={guide.id} className="group">
                      <Card className="h-full overflow-hidden transition-all hover:shadow-md">
                        <div className="relative aspect-video overflow-hidden">
                          ...
                          <Badge className="absolute right-3 top-3">{guide.tag}</Badge>
                        </div>
                        <CardHeader className="p-4">
